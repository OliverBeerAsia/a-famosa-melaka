...
function main() {
  ...
  // UI CHROME MOVED. Panels, frames, buttons, slots, the coin and the seal are
  // now generated by `node tools/forge/ui.cjs` at NATIVE resolution on the
  // 50-colour canon, and shipped at an exact x3 upscale. The generators below
  // are the pre-Stage-6 CSS-pixel-density versions: running them would clobber
  // the kit with off-canon art and fail `npm run validate:canon`. They are kept
  // behind --legacy-chrome purely for reference/diffing.
  if (process.argv.includes('--legacy-chrome')) {
    console.log('--- Legacy chrome (SUPERSEDED by tools/forge/ui.cjs) ---');
    generateParchment();
    ...
    console.log('--- UI chrome: skipped (owned by tools/forge/ui.cjs) ---');
  }
